...
using namespace std;
int n, m, ans, s, t, cnt = 2, hd[2000500], to[2000500], w[2000500],
                     dis[2000500], nxt[2000500], g[150][150];
  ...
  for (int i = 1; i <= n; i++) {
    for (int j = 1; j <= m; j++) {
      ...
      for (int d : {0, 1, 2, 3}) {
        ...
        if (inR(nx, ny) && g[nx][ny] != 2 && g[i][j] != 1) {
          addE(h(i, j), h(nx, ny), 1);
          addE(h(nx, ny), h(i, j), 0);
        }
      }
    }
  }
